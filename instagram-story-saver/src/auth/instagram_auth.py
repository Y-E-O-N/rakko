...
class InstagramAuth:
    ...
    def _setup_client(self):
        ...
        if self.proxy:
            try:
                ...
                logger.debug(f"프록시 설정됨")
            except Exception as e:
                logger.warning(f"프록시 설정 실패: {e}")
        
        self.client.delay_range = [self.delay_min, self.delay_max]
    ...
    def _fresh_login(self) -> bool:
        ...
        try:
            ...
            logger.error("2단계 인증이 필요합니다. totp_secret을 설정하세요.")
            return False
        except ChallengeRequired:
            logger.error("Instagram 보안 확인이 필요합니다. 웹/앱에서 확인 후 다시 시도하세요.")
            return False
        except BadPassword:
    ...
    def _check_session_file_permissions(self):
        ...
        try:
            ...
            if mode & (stat.S_IRGRP | stat.S_IWGRP | stat.S_IROTH | stat.S_IWOTH):
                logger.warning(
                    f"세션 파일 권한이 너무 개방적입니다. "
                    f"'chmod 600 {self.session_file}'를 실행하세요."
                )
        except (OSError, AttributeError):
            pass
    ...
    def is_logged_in(self) -> bool:
        ...
        try:
            self.client.get_timeline_feed()
            return True
        except (LoginRequired, ClientConnectionError, ConnectionError, ClientError):
            return False
        except Exception as e:
...
class SessionManager:
    ...
    def ensure_logged_in(self) -> bool:
        ...
        if self.consecutive_failures >= self.max_failures:
            ...
            logger.error(f"연속 {self.max_failures}회 로그인 실패, {self.cooldown_seconds // 60}분 후 재시도")
            self.consecutive_failures = 0
            return False
    ...
    def handle_api_error(self, error: Exception) -> bool:
        ...
        if isinstance(error, PleaseWaitFewMinutes) or "please wait" in error_str or "rate limit" in error_str:
            self.cooldown_until = time.time() + self.cooldown_seconds
            logger.warning(f"API 제한, {self.cooldown_seconds // 60}분 대기")
            return False
